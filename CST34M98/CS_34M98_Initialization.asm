...
F_Init_SOC:
	...
	;-----------------------------------------------------------------	
	movlw		00110010B			;MCK:(上电默认值为:00100010)
...
;	call		F_SPI_Init
	...
	movlw		00101000b			;ADCFG:(上电默认值为:00000000)
	...
	movlf		10,R_SYS_A0		;延时5ms(@cpuclk=2MHz)
	call		F_delay_1kclk	
	...
	;设置LVD
	;-----------------------------------------------------------------
	movlw		00100010B			;AD2OH:(上电默认值为:001000X0)
	movwf		LVDCON				;Bit7   LVDEN: 低电压检测使能信号
									;	0 = 低电压检测不使能*
									;	1 = 低电压检测使能
...
;	bsf			INTE2,UR0_RNIE		;串口0接收FIFO非空中断使能 --> <详见Datasheeet>
	bcf			INTE3,RTCIE			;使能RTC中断
	;
	;
	;
...
F_Init_IO:
	...
	;BLE_TX		   = output/no-pull/low		--> PT1.7
	;BLE_RX		   = output/pull/high		--> PT1.6
	...
	;XINL		   = NC						--> PT1.0
	;-------------------------------	
	MOVLW		10011000B			;PT1EN:(上电默认值:00000000)
	MOVWF		PT1EN 				;	0 = 定义为输入口
									;	1 = 定义为输出口
	
	MOVLW		01011000B			;PT1PU:(上电默认值:00000000)	
	MOVWF		PT1PU				;	0 = 断开上拉电阻
									;	1 = 使用上拉电阻

	MOVLW		01100000B			;PT1:(上电默认值:xxxxxxxx)			
	MOVWF		PT1			

	MOVLW		11111110B			;AIENB:(上电默认值:11111111)			
	...
	;PT2端口
	;-------------------------------
	;Status(S2M	)		= input/no-pull/low	     --> PT2.7
	;BackLight      = output/pull/high	 	 --> PT2.6 ble status 输入
	;Key		    =  NC				     --> PT2.5
	;PT2.4			= output/pull/high        --> PT2.4
	;PT2.3			= output/no-pull/high    --> PT2.3
	;PT2.2          = output/no-pull/high    --> PT2.2
	...
	MOVLW		00000000B			;PT2:(上电默认值:xxxxxxxx) 	
	MOVWF		PT2
	
	;-------------------------------
	;PT3端口  pt3.1 pt3.2设置成上拉输入
	;-------------------------------	
	MOVLW		11111001B			;PT3EN:(上电默认值:00000000)
	MOVWF		PT3EN 				;	0 = 定义为输入口
									;	1 = 定义为输出口	
	
	MOVLW		00000110B			;PT3PU:(上电默认值:00000000)
	...
	MOVWF		PT5EN				;Bit7	SMT5_EN: PT5[4:0]输入施密特使能控制信号
									;	0 = PT5[4:0]输入施密特使能关闭
	...
	MOVLW		00000000B			;PT5:(上电默认值:000xxxxx)	
	MOVWF		PT5		
	...
	MOVWF		PTCON				;Bit7	BZEN: 蜂鸣器使能标志，默认不使能
